...
class CityscapesCocoMix(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        record["width"] = city_mix_image.shape[2]   ##############数据来源有问题，似乎是一样的
        ...
        # record["image"] = np.transpose(city_mix_image,(1,2,0)) * numpy.array([0.229, 0.224, 0.225]) + numpy.array([0.485, 0.456, 0.406])
        record["image"] = image_.permute(2,0,1)
##############################
        obj = []
        mask = (city_mix_target == 254)
        points = np.argwhere(mask)
        # res = Instances([city_mix_image.shape[1], city_mix_image.shape[2]])
        # exit()
        if len(points) > 0:
            # 计算包围框
