...
def get_score_dataset_dicts(city_dir, coco_dir, split_):
    train_dataset = CityscapesCocoMix(split='train', preprocess=Preprocess(),
                                    cs_root=city_dir, coco_root=coco_dir)
    split_ratio = 0.9
    total_samples = len(train_dataset)
...
def register_score_dataset():
    for d in ["train", "val"]:
        DatasetCatalog.register("OE_dataset_" + d,  lambda x = city_dir, y=coco_dir, d=d: get_score_dataset_dicts(x,y,d))
        MetadataCatalog.get("OE_dataset" + d).set(thing_classes=["OOD"])
    balloon_metadata = MetadataCatalog.get("OE_dataset_train")
