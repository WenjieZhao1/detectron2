...
city_dir = '/people/cs/w/wxz220013/OOD/synboost/final_dataset/city_scape'
coco_dir = '/people/cs/w/wxz220013/OOD/meta-ood/COCO'
...
class Preprocess(object):
    ...
    def __call__(self, city_img, city_gt, ood_img, ood_gt, anomaly_mix_or_not):
        # plt.imshow(city_img.astype(numpy.uint8))
        # plt.savefig('./1.jpg', dpi=300)
        city_img, city_gt = self.inlier_transform(city_img, city_gt)
        # pic = city_img.permute(1,2,0)*(numpy.array([0.229, 0.224, 0.225])) + numpy.array([0.485, 0.456, 0.406])
        # pic = pic*255
        # plt.imshow(pic.astype(numpy.uint8))
...
class CityscapesCocoMix(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        record["annotations"] = obj
            # image = torch.from_numpy(city_mix_image).permute(1,2,0)
            # image = (image*image_std + image_mean)*255
            # image = image.numpy().astype(numpy.uint8)
            # # 画出包围框
            # cv2.rectangle(image, (min_y, min_x), (max_y, max_x), (0, 255, 0), 2)
            # # 保存结果图像
            # cv2.imwrite(str(i) + "output_image.jpg", image)
        return city_mix_image, city_mix_target
...
for d in ["train", "val"]:
    DatasetCatalog.register("OE_dataset" + d,  lambda x = city_dir, y=coco_dir, d=d: get_OE_dataset_dicts(x,y,d))
    MetadataCatalog.get("OE_dataset" + d).set(thing_classes=["OOD"])
balloon_metadata = MetadataCatalog.get("OE_dataset_train")

dataset_dicts = CityscapesCocoMix(split='train', preprocess=Preprocess(),cs_root=city_dir, coco_root=coco_dir)

for i in range(1,4):
    d = dataset_dicts[(i,i)]
    city_mix_image, city_mix_target = d
    print(city_mix_target.shape)
    mask = (city_mix_target == 254)
    print(type(city_mix_target))
    points = np.argwhere(mask)
    print(len(points))
    exit()
    if len(points) > 0:
